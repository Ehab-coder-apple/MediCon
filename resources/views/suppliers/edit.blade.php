                    ...
                    <form action="{{ route($routePrefix . 'suppliers.update', $supplier) }}" method="POST">
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="notes" id="notes" rows="3" 
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('notes', $supplier->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                <div>
                                    ...
                                    <span class="ml-1 font-medium">{{ $supplier->total_purchases }}</span>
                                </div>
                                <div>
                                    <span class="text-gray-500">Total Amount:</span>
                                    <span class="ml-1 font-medium">${{ number_format($supplier->total_purchase_amount, 2) }}</span>
                                </div>
                                <div>
                                    <span class="text-gray-500">Last Purchase:</span>
                                    <span class="ml-1">{{ $supplier->last_purchase_date ?? 'Never' }}</span>
                                </div>
                            </div>
                        </div>
